...
public class GenerateSmsCdrCommand implements Command {
    ...
    public void execute(Properties properties) throws Exception {
        ...
        ConsoleHelper.writeMessage("Генерация cdr смс");
        ...
        String filePath = properties
                .getProperty("path", "../") + "mobile_calls__" + now.format(formatter) + ".cdr";

        try (PrintWriter writer = new PrintWriter(new FileWriter(filePath, true))) {
            for (int i = 0; i < count; i++) {
                writer.write(new SmsCdr(properties).createMessageCdr());
                if (i % (count / 25) == 0) {
